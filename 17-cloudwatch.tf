...
resource "aws_cloudwatch_metric_alarm" "lab2_alb_5xx_alarm01" {
  ...
  dimensions = {
    LoadBalancer = aws_lb.lab2_alb.arn_suffix
  }
...
# Explanation: Dashboards are your cockpit HUD — lab2 wants dials, not vibes.
resource "aws_cloudwatch_dashboard" "lab2_dashboard01" {
    ...
    widgets = [
      {
        ...
        properties = {
          metrics = [
            ["AWS/ApplicationELB", "RequestCount", "LoadBalancer", aws_lb.lab2_alb.arn_suffix],
            [".", "HTTPCode_ELB_5XX_Count", ".", aws_lb.lab2_alb.arn_suffix]
          ]
          period = 300
          stat   = "Sum"
          region = var.aws_region
          title  = "lab2 ALB: Requests + 5XX"
        }
      },
      {
        type   = "metric"
        ...
        properties = {
          metrics = [
            ["AWS/ApplicationELB", "TargetResponseTime", "LoadBalancer", aws_lb.lab2_alb.arn_suffix]
          ]
          period = 300
          stat   = "Average"
          region = var.aws_region
          title  = "lab2 ALB: Target Response Time"
        }
      }
    ]
  })
