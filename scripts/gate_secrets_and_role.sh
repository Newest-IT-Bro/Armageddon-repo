...
usage() {
  cat <<EOF
Usage:
  REGION=eu-west-2 INSTANCE_ID=i-... SECRET_ID=my-secret ./gate_secrets_and_role.sh
...
Examples:
  REGION=eu-west-2 INSTANCE_ID=i-0c5aa4f712c66ce1b SECRET_ID=lab/rds/mysql ./gate_secrets_and_role.sh

  # Strict rotation requirement:
  REQUIRE_ROTATION=true REGION=eu-west-2 INSTANCE_ID=i-0c5aa4f712c66ce1b SECRET_ID=lab/rds/mysql ./gate_secrets_and_role.sh

  # Run on the EC2 and verify it can read the secret value (does NOT print it):
  CHECK_SECRET_VALUE_READ=true REGION=eu-west-2 INSTANCE_ID=i-0c5aa4f712c66ce1b SECRET_ID=lab/rds/mysql ./gate_secrets_and_role.sh
EOF
}
