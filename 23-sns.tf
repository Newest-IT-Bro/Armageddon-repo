...
resource "aws_sns_topic_subscription" "lab1cbs_ir_lambda_sub" {
  topic_arn = aws_sns_topic.db_incidents.arn
  protocol  = "lambda"
  endpoint  = aws_lambda_function.lab1cbs_ir_lambda01.arn
}
...
resource "aws_lambda_permission" "lab1cbs_allow_sns_invoke" {
  ...
  function_name = aws_lambda_function.lab1cbs_ir_lambda01.function_name
...
output "lab1cbs_ir_reports_bucket" {
  value = aws_s3_bucket.lab1cbs_ir_reports_bucket.bucket
}