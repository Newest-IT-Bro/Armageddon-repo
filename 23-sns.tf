...
resource "aws_sns_topic_subscription" "lab2_ir_lambda_sub" {
  topic_arn = aws_sns_topic.db_incidents.arn
  protocol  = "lambda"
  endpoint  = aws_lambda_function.lab2_ir_lambda01.arn
}


# Allow SNS to invoke Lambda (source_arn must be the same topic)
resource "aws_lambda_permission" "lab2_allow_sns_invoke" {
  statement_id  = "AllowExecutionFromSNS"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lab2_ir_lambda01.function_name
  principal     = "sns.amazonaws.com"
  source_arn    = aws_sns_topic.db_incidents.arn
}

# Output report bucket (must exist with this exact name)
output "lab2_ir_reports_bucket" {
  value = aws_s3_bucket.lab2_ir_reports_bucket.bucket
}