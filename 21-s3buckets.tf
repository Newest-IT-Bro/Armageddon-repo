...
# Explanation: Encrypt logs at rest—lab1cbs protects the black box with cryptography.
resource "aws_s3_bucket_server_side_encryption_configuration" "lab1cbs_alb_logs_encryption" {
    ...
    Statement = [
      {
        ...
        Action   = "s3:PutObject"
        Resource = "${aws_s3_bucket.lab1cbs_alb_logs_bucket.arn}/${var.alb_access_logs_prefix}/AWSLogs/${data.aws_caller_identity.current.account_id}/*"
        Condition = {
          StringEquals = {
      ...
      },
      {
        ...
        Resource = aws_s3_bucket.lab1cbs_alb_logs_bucket.arn
      }
    ]
  })
}
