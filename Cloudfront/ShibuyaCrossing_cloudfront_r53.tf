...
resource "aws_route53_record" "ShibuyaCrossing_apex_to_cf01" {
  zone_id = local.hosted_zone_id
  name    = var.domain_name
  type    = "A"

  alias {
    name                   = aws_cloudfront_distribution.ShibuyaCrossing_cf01.domain_name
    zone_id                = aws_cloudfront_distribution.ShibuyaCrossing_cf01.hosted_zone_id
